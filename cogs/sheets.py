...
busyLocs = [[99, "LM"], [100, "SP"]]
forbiddenLocs = [[2, "BU"], [84, "LM"]]
highestWorld = 141
...
async def update_sheet_row(agcm, ports_row, timestamp, name, isRank):
    ...
    cell_list = [gspread_asyncio.Cell(21, i+1, value=val) for i, val in enumerate(ports_row)]
    mobile_cell_list = [gspread_asyncio.Cell(32+i, 2, value=val) for i, val in enumerate(ports_row)]
...
async def write_error(agcm, name, date, msg):
    ...
    for i, e in enumerate(errors):
        if e == "":
            row = i+1
            cell_list = [gspread_asyncio.Cell(row, col, value=values[col-1]) for col in range(1,4)]
            await sheet.update_cells(cell_list, nowait=True)
            return
        elif i == len(errors)-1:
...
class Sheets(commands.Cog):
    ...
    @commands.command(name='toggle_dxp', hidden=True)
    @portables_admin()
    async def toggle_dxp(self, ctx):
        '''
        Toggle the restriction that a location can have at most 3 portables. (Owner only)
        '''
        addCommand()
        await ctx.channel.trigger_typing()

        global dxp_active
        dxp_active = not dxp_active
        ...
        if not dxp_active:
            await home.update_cell(18, 8, "1", nowait=True)
            await ctx.send('DXP mode disabled.')
        else:
            await home.update_cell(18, 8, "0", nowait=True)
            await ctx.send('DXP mode enabled.')
